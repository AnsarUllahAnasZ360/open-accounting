...
|---|---|---|---|
| Product spec §4 / §6.8 | M10 ships a real Bedrock categorization action, report-backed deterministic acceptance shortcuts, a Convex AI SDK read-tool action, and streamed HTTP delivery, not the full Vercel AI SDK `useChat` approval/tool UI. | The safe increment proves confirm-first rule creation, provider/degraded state, registry readiness, Settings AI SDK runtime smoke test, full-page access, server-authorized read tools, free-form read-tool routing through Bedrock, and ledger-safe routing; the remaining write tools are intentionally not auto-executed from chat. | Wire the remaining confirm-card action tools (`categorizeTransactions`, `draftInvoice`, `addBill`, `createJournalEntry`) into the chat approval UI, then mark M10 fully complete after e2e evidence. |
