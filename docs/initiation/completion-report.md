...
|---|---|---|---|
| Product spec §4 / §6.8 | M10 ships a real Bedrock categorization action plus report-backed deterministic chat drawer/full-page mode, not full Vercel AI SDK streaming with Bedrock chat/tool calls. | The safe increment proves confirm-first actions, provider/degraded state, registry readiness, Settings AI SDK runtime smoke test, full-page access, and ledger-safe routing; the Bedrock categorizer now produces structured proposals but chat remains deterministic/report-backed. | Wire AI SDK streaming `useChat`, server-side read/action tools, and route chat tool calls through Bedrock before marking M10 fully complete. |
