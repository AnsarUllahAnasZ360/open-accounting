...
function CommandDialog({
  ...
  description = "Search transactions, contacts, reports, and actions.",
  children,
  className,
  ...props
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "relative flex cursor-default items-center gap-2 rounded-[10px] px-2 py-2 text-sm outline-hidden select-none data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        className,
      )}
      {...props}
    />
...
function CommandShortcut({
  ...
  return (
    <span
      ...
      className={cn(
        "money-figures ml-auto inline-flex h-5 shrink-0 items-center rounded-md border border-border bg-background px-1.5 text-[11px] font-medium leading-none text-muted-foreground shadow-xs",
        className,
      )}
      {...props}
    />
