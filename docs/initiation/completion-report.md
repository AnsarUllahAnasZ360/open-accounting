...
| 14 | Chat answers 5 questions correctly + confirmed action posts | WORKING | `docs/initiation/evidence/2026-06-11-m10-action-tools-e2e.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-production-e2e.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-verify.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-convex-deploy.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-vercel-deploy.txt`; `docs/initiation/evidence/2026-06-11-m13-e2e-production-final-green.txt`; `docs/initiation/evidence/2026-06-11-m10-ai-chat.png`; `docs/initiation/evidence/2026-06-11-m10-ai-chat-full-page.png`; `docs/initiation/evidence/2026-06-11-m10-chat-read-tools-unit.txt`; `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-unit.txt`; `docs/initiation/evidence/2026-06-11-m10-live-eval-result.json` | Report-backed deterministic answers, full-page Ask AI, streamed Bedrock read-tool runtime, degraded mode, confirmed rule creation, and the full confirm-card action set are evidenced. Production Playwright confirms `categorizeTransactions`, `createRule`, `draftInvoice`, `addBill`, and `createJournalEntry`; bill/journal postings delegate to `postEntry`, draft invoices do not post revenue, and categorization routes through the pipeline. |
...
| 19 | `pnpm verify` + `pnpm test:e2e` green; eval accuracy reported (goal.md §2; ≥80% is a target, not a blocker) | WORKING | `docs/initiation/evidence/2026-06-11-m10-action-tools-closeout-verification.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-verify.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-full-e2e-rerun.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-e2e.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-production-e2e.txt`; `docs/initiation/evidence/2026-06-11-m13-e2e-production-final-green.txt`; `docs/initiation/evidence/2026-06-11-m10-live-eval-result.json`; `docs/initiation/evidence/2026-06-11-m10-live-eval-production-e2e.txt` | Current `pnpm verify` is green with 17 unit files / 75 tests, current full local/dev `pnpm test:e2e` is 17/17, current production-domain AI chat focused e2e is 3/3, and prior full production e2e remains evidenced; the owner-authenticated seeded eval is 120/120 = 100.0%, above the 80.0% target. |
| 20 | Production URL live, owner login in prod (goal.md §1.9) | WORKING | `docs/initiation/evidence/2026-06-11-m10-action-tools-closeout-verification.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-vercel-deploy.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-http-check.txt`; `docs/initiation/evidence/2026-06-11-m10-action-tools-production-e2e.txt`; `docs/initiation/evidence/2026-06-11-m13-http-checks-after-seed-job-and-plaid-fixes.txt`; `docs/initiation/evidence/2026-06-11-m13-e2e-production-final-green.txt`; `docs/initiation/evidence/2026-06-11-prototype-copy-http-check.txt` | `https://openbooks.ansarullahanas.com` is live, `/ask-ai` returns HTTP 200 on the custom domain, and the production-domain chat acceptance signs in and passes 3/3 after the action-tools deploy. |
...
|---|---|---|---|
| Product spec §4 / §6.8 | M10 uses the existing OpenBooks chat component plus an authenticated Convex HTTP stream rather than replacing the UI with Vercel AI SDK `useChat` primitives. | The shipped path now proves provider/degraded state, Bedrock read-tool streaming, full-page access, server-authorized read tools, and confirm-card actions for `categorizeTransactions`, `createRule`, `draftInvoice`, `addBill`, and `createJournalEntry` without letting the model auto-execute ledger writes. | No M10 blocker remains. A later UI refactor can adopt `useChat` if it reduces maintenance, but the current contract is evidenced through local and production Playwright. |
...
### 2026-06-11 13:58 CDT — M10 AI chat stream runtime

What changed:

- Added `convex/aiChatRuntime.ts`, a Convex Node action that calls AI SDK Bedrock `generateText` with the five server-authorized read tools.
- Added `/ai/chat` as an authenticated Convex HTTP route with restricted app-origin CORS, bearer-token auth, and chunked text delivery.
- Wired the existing drawer and full-page Ask AI component to stream free-form live read questions through the Convex route when Bedrock is active.
- Kept the five acceptance shortcuts deterministic so the owner-facing examples remain stable and report-backed; write-like prompts still render confirmation cards instead of executing through the model.
- Deployed both Convex production functions and the Vercel production frontend.

Evidence:

- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-unit.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-convex-dev-once.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-verify.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-e2e.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-convex-deploy.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-vercel-deploy.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-http-check.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-cors-check.txt`
- `docs/initiation/evidence/2026-06-11-m10-chat-stream-runtime-production-e2e.txt`

Verification:

- `pnpm test:unit -- convex/aiChatRuntime.test.ts convex/aiChatTools.test.ts` green: 15 files / 68 tests.
- `npx convex dev --once --typecheck=disable` green for local Convex codegen/load.
- `pnpm verify` green: typecheck, lint, Next.js production build, and 15 unit files / 68 tests.
- `pnpm exec playwright test tests/e2e/ai-chat.spec.ts` green: 3/3 passed.
- `npx convex deploy --yes --typecheck=disable` green for production Convex.
- `vercel deploy --prod --yes` green: production deployment `dpl_BwgozpA6Syd2eSdYasLD6yAPnyRq` ready.
- `curl -I https://openbooks.ansarullahanas.com/ask-ai` returned HTTP 200.
- `curl -i -X OPTIONS https://perceptive-guanaco-487.convex.site/ai/chat` returned HTTP 204 with `access-control-allow-origin: https://openbooks.ansarullahanas.com`.
- `PLAYWRIGHT_BASE_URL=https://openbooks.ansarullahanas.com pnpm exec playwright test tests/e2e/ai-chat.spec.ts --grep "full-page"` green: 1/1 passed.
...
| Item | Status | Notes |
|---|---:|---|
| Convex AI SDK read-tool runtime | PASS | Bedrock `generateText` can call the five authorized read tools from a Convex Node action. |
| External-call boundary | PASS | Bedrock remains in Convex actions; the frontend only calls the authenticated Convex HTTP route. |
...
- `docs/initiation/evidence/2026-06-11-prototype-copy-vercel-deploy.txt`
- `docs/initiation/evidence/2026-06-11-prototype-copy-http-check.txt`
- `docs/initiation/evidence/2026-06-11-prototype-copy-production-render.png`
...
- `vercel deploy --prod --yes` green: production deployment `dpl_4tEmcA1c2teWejieRrdumEKf9tYR`.
- `curl -I https://openbooks.ansarullahanas.com` returned HTTP 200.
- Browser automation opened `https://openbooks.ansarullahanas.com/` and confirmed the same prototype phrases plus request-access copy.
...
| Production redeploy | PASS | Vercel production deployment `dpl_4tEmcA1c2teWejieRrdumEKf9tYR` is ready and the custom domain returns HTTP 200. |

### 2026-06-11 14:39 CDT — M10 AI chat action tools

What changed:

- Added `convex/aiChatActions.ts` with the remaining confirm-card write tools: `categorizeTransactions`, `draftInvoice`, `addBill`, and `createJournalEntry`.
- Kept the OpenBooks invariant intact: transaction categorization routes through the existing pipeline, bill and journal postings delegate to `ledger.postEntry`, and draft invoices create draft records without posting revenue.
- Hardened account selection so an AI-confirmed action cannot use an account with the wrong type/subtype, even when an explicit account number is supplied.
- Extended the shared Ask AI drawer/full-page component so each proposal card carries a typed action payload, visible owner-review facts, and a confirm button that calls the matching Convex mutation.
- Expanded Playwright coverage so the browser now confirms `createRule`, `categorizeTransactions`, `draftInvoice`, `addBill`, and `createJournalEntry`, then re-checks mobile and full-page chat.
- Deployed the updated Convex production functions and Vercel production frontend.

Evidence:

- `docs/initiation/evidence/2026-06-11-m10-action-tools-convex-dev-once.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-verify.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-e2e.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-auth-rerun.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-full-e2e.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-full-e2e-rerun.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-convex-deploy.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-vercel-deploy.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-http-check.txt`
- `docs/initiation/evidence/2026-06-11-m10-action-tools-production-e2e.txt`
- `docs/initiation/evidence/2026-06-11-m10-ai-chat.png`
- `docs/initiation/evidence/2026-06-11-m10-ai-chat-mobile.png`
- `docs/initiation/evidence/2026-06-11-m10-ai-chat-full-page.png`
...
- `npx convex dev --once --typecheck=disable` green for local function load and codegen.
- `pnpm verify` green: typecheck, lint, Next.js production build, and 17 unit files / 75 tests.
- `pnpm test:e2e -- tests/e2e/ai-chat.spec.ts` green: 3/3 passed locally.
- The first full `pnpm test:e2e` run was 16/17 after a stale auth selector expected exactly "Ask AI"; the app rendered the prototype-aligned "Ask AI cmd J" button. `tests/e2e/auth.spec.ts` now accepts the shortcut text and the focused auth rerun is 3/3.
- Full `pnpm test:e2e` rerun is green: 17/17 passed locally.
- Closeout rerun after resume found and fixed one M5 post-seed timing race: dashboard assertions now wait up to 30 seconds for the loaded seeded dashboard. Focused `tests/e2e/core-screens.spec.ts` passed 1/1, full `pnpm test:e2e` passed 17/17, and `pnpm verify` remained green.
- `npx convex deploy --yes --typecheck=disable` green for production Convex.
- `vercel deploy --prod --yes` green: production deployment `dpl_7tWnXasuTqNSWATB9irgZmQbstFY` ready.
- `curl -I https://openbooks.ansarullahanas.com/ask-ai` returned HTTP 200.
- `PLAYWRIGHT_BASE_URL=https://openbooks.ansarullahanas.com pnpm test:e2e -- tests/e2e/ai-chat.spec.ts` green: 3/3 passed on the production custom domain.

PASS/PARTIAL table:

| Item | Status | Notes |
|---|---:|---|
| `categorizeTransactions` confirm card | PASS | Browser-confirmed; server routes matching transactions through `pipeline.recategorizeTransaction`. |
| `createRule` confirm card | PASS | Existing confirmed-rule path remains green and writes audit-visible AI rule events. |
| `draftInvoice` confirm card | PASS | Creates draft invoice/contact records only; no revenue journal entry posts from the draft action. |
| `addBill` confirm card | PASS | Posts an open bill to A/P through `ledger.postEntry`; account-shape guard is unit-tested. |
| `createJournalEntry` confirm card | PASS | Posts balanced debit/credit lines through `ledger.postEntry`; unit and browser tests cover the path. |
| Mobile/full-page chat regression | PASS | Focused local and production Playwright keep mobile drawer and `/ask-ai` full-page answers green. |
| M10 chat checkbox | PASS | Full read-tool, streaming, full-page, inline artifact, suggested prompt, Explain report, and confirm-card action set is now evidenced. |
