...
Expected substring: "Demo seed complete."
...
- main:
  ...
  - button "Export CSV bundle" [disabled]
  - button "Export JSON" [disabled]
  - button "Reset demo data" [disabled]
  - text: Demo books have not been seeded in this workspace yet.
  ...
  - table:
    - rowgroup:
      - row "Account Type Debit Credit":
        ...
        - columnheader "Credit"
    - rowgroup:
      - row "1000 · Cash on Hand asset $1,111.05 $0.00":
      ...
      - row "1100 · Accounts Receivable asset $20.00 $0.00":
        ...
        - cell "$20.00"
        - cell "$0.00"
      - row "1150 · Stripe Clearing asset $233,744.24 $0.00":
      ...
      - row "2500 · Loans Payable liability $0.00 $0.00":
        ...
        - cell "$0.00"
        - cell "$0.00"
      - row "3000 · Owner's Equity equity $0.00 $1,111.05":
      ...
      - row "3900 · Opening Balance Equity equity $0.00 $0.00":
        ...
        - cell "$0.00"
        - cell "$0.00"
      - row "4000 · Sales income $0.00 $242,482.01":
        - cell "4000 · Sales"
        - cell "income"
        - cell "$0.00"
        - cell "$242,482.01"
      ...
      - row "5500 · Professional Services expense $0.00 $0.00":
        ...
        - cell "$0.00"
        - cell "$0.00"
      - row "5600 · Payment Processing Fees expense $7,165.97 $0.00":
  ...
  - text: Link launch
  - paragraph: Create a sandbox Link token or use fixture bypass.
  - text: Pipeline sync
  - paragraph: Fixture sync sends Plaid-shaped transactions to stages 1-3.
  - button "Prepare Link"
  - button "Use sandbox bypass"
  - button "Simulate relink"
  - text: Transactions sync fixture
  - paragraph: Uses Plaid-shaped added transactions with personal finance category priors.
  - button "Sync fixture"
  - text: Plaid Sandbox Checking checking ending 0000 $4,250.00 Plaid Sandbox Credit Card credit ending 1111 -$87.90 Recent bank imports
  - paragraph: These Plaid-shaped items were staged as OpenBooks transactions through the pipeline.
  - text: Plaid Sandbox Bank 2026-06-12 · needs review · Plaid prior -$18.75 Client ACH 2026-06-11 · needs review · Plaid prior $1,250.00 Notion 2026-06-10 · needs review · Plaid prior -$49.99 Plaid Sandbox Bank needs you to sign in again. Relink item fixture-item-login-required in update mode. AI
  ...
  - text: Acme Studio LLC services · USD Demo
  ...
  - text: 1 Rent If merchant contains "Maple Yard" and money out -> Rent, auto-post 11 hits
  - button "Edit"
  - text: 2 Cloud infrastructure If description contains "AWS" and money out -> Cloud/Infrastructure, auto-post 31 hits
  - button "Edit"
  - text: 3 Marketing ads If merchant contains "Ads" and money out -> Marketing & Ads, auto-post 88 hits
  - button "Edit"
  - text: 4 Software subscriptions If description contains "subscription" and money out -> Software & SaaS, auto-post 79 hits
  - button "Edit"
  - text: 5 Bank fees If merchant contains "Mercury Bank Fee" and money out -> Bank Fees, auto-post 11 hits
  - button "Edit"
  - text: 6 Stripe fees If merchant contains "Stripe Fees" and money out -> Payment Processing Fees, auto-post 10 hits
  - button "Edit"
  - text: Audit log
...
```ts
  ...
  48  |   await page.getByRole("button", { name: "Reset demo data" }).click();
> 49  |   await expect(page.getByTestId("demo-seed-message")).toContainText("Demo seed complete.", {
      |                                                       ^ Error: expect(locator).toContainText(expected) failed
  50  |     timeout: 180_000,
  51  |   });
  52  |   await expect(demoExport).toBeEnabled({ timeout: 30_000 });
  53  |   const aiSettings = page.getByTestId("m10-ai-settings");
  54  |   await expect(aiSettings).toBeVisible({ timeout: 15000 });
  55  |   await expect(aiSettings.getByText(/AI provider is not configured|Bedrock provider is configured/)).toBeVisible();
  56  |   await expect(aiSettings.getByText(/Degraded mode|Bedrock active/)).toBeVisible();
  57  |   await expect(page.getByRole("radio", { name: /Balanced/ })).toBeChecked();
  58  |   await expect(page.getByRole("radio", { name: /Suggest everything/ })).toBeVisible();
  59  |   await expect(page.getByRole("radio", { name: /Balanced/ })).toBeVisible();
  60  |   await expect(page.getByRole("radio", { name: /Autopilot/ })).toBeVisible();
  61  |   await page.getByRole("button", { name: "Test AI connection" }).click();
  62  |   await expect(
  63  |     aiSettings.getByText(/Bedrock provider is configured|Bedrock env is absent|AI provider is not configured/),
  64  |   ).toBeVisible();
  65  |   await page.screenshot({
  66  |     path: "docs/initiation/evidence/2026-06-11-m10-ai-settings.png",
  67  |     fullPage: true,
  68  |   });
  69  |
  70  |   await page.goto("/reports");
  71  |   await expect(page.getByTestId("reports-screen")).toBeVisible({ timeout: 15000 });
  72  |   await expect(page.getByRole("button", { name: "Export CSV" })).toBeEnabled({ timeout: 15000 });
  73  |   await page.getByRole("button", { name: "Explain report" }).click();
  74  |   const chatDrawer = page.getByTestId("m10-ai-chat-drawer");
  75  |   await expect(chatDrawer).toBeVisible();
  76  |   await expect(chatDrawer.getByText(/Degraded mode|Bedrock active/)).toBeVisible();
  77  |   await expect(chatDrawer.getByText("Explain this report")).toBeVisible();
  78  |
  79  |   await page.getByRole("button", { name: "Top 5 expenses this quarter?" }).click();
  80  |   await expect(chatDrawer.getByText("Top expense categories").first()).toBeVisible({ timeout: 15000 });
  81  |   await expect(chatDrawer.getByTestId("ai-answer-table").locator("tbody tr").first()).toBeVisible();
  82  |
  83  |   await page.getByPlaceholder("Ask about your books").fill("Create a rule for Uber");
  84  |   await page.getByRole("button", { name: "Send question" }).click();
  85  |   await expect(chatDrawer.getByText("Proposed action").first()).toBeVisible();
  86  |   await expect(chatDrawer.getByRole("button", { name: "Confirm rule" })).toBeVisible();
  87  |   await expect(chatDrawer.getByText("Nothing has been posted or written yet.")).toBeVisible();
  88  |   await chatDrawer.getByRole("button", { name: "Confirm rule" }).click();
  89  |   await expect(chatDrawer.getByTestId("ai-proposal-result")).toContainText(/Rule (created|updated); Uber/, {
  90  |     timeout: 15000,
  91  |   });
  92  |   await page.screenshot({
  93  |     path: "docs/initiation/evidence/2026-06-11-m10-ai-chat.png",
  94  |     fullPage: true,
  95  |   });
  96  |
  97  |   await page.goto("/settings");
  98  |   await expect(page.getByText("AI confirmed: Uber")).toBeVisible({ timeout: 15000 });
  99  | });
  100 |
```